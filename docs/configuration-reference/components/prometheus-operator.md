...
| `watch_labeled_prometheus_rules` | By default prometheus operator watches only the PrometheusRule objects in the cluster that are labeled `release: "prometheus-operator"`. If set to `false` then all the PrometheusRule will be watched. | `true` | false |
