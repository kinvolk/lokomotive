...
* A Lokomotive cluster accessible via `kubectl` [deployed on Equinix Metal](../quickstarts/equinix-metal.md).

* IPv4 address pools for MetalLB to allocate — one address per LoadBalancer Service. On Equinix Metal, you need to create [Public Elastic IPs](https://metal.equinix.com/developers/docs/networking/elastic-ips/).
