---
title: Updating bootstrap kubelet
weight: 10
---
...
> update: `kubectl version -ojson | jq -r '.serverVersion.gitVersion'`.
...
To finish the update process, uncordon updated node to mark it as schedulable for pods, using
...
- When updating kubelet on nodes which are running Rook Ceph, verify that the Ceph cluster is in
  the **`HEALTH_OK`** state. If it is in any other state, **do not proceed with the update** as
