...
resource "null_resource" "copy-controller-secrets" {
  ...
  connection {
    type    = "ssh"
    host    = oci_core_instance.controllers[count.index].public_ip
    user    = "core"
    timeout = "15m"
  }
  ...
  triggers = {
    controller_id = oci_core_instance.controllers[count.index].id
  }
}
...
resource "null_resource" "bootkube-start" {
  ...
  connection {
    type    = "ssh"
    host    = oci_core_instance.controllers[0].public_ip
    user    = "core"
    timeout = "15m"
  }
