...
// Package equinixmetal provides the implenentation of the Platform interface
// for Equinix Metal cloud provider.
...
const (
	// Name represents Equinix Metal platform name as it should be referenced in function calls and configuration.
	Name = "equinixmetal"
)
...
func createTerraformConfigFile(cfg *config, terraformPath string) error {
	...
	// Equinix Metal does not accept tags as a key-value map but as an array of
	// strings.
	...
	steps := []terraform.ExecutionStep{
		// We need the controllers' IP addresses before we can apply the 'dns' module.
		{
			...
			Args: append([]string{
				"apply",
				"-auto-approve",
				fmt.Sprintf("-target=module.equinixmetal-%s.metal_device.controllers", c.ClusterName),
			}, extraArgs...),
		},
		{
...
func (c *config) terraformAddDeps() {
	...
	// "module.equinixmetal" instead of "module.equinixmetal.device_ids") it
	// seems to work fine. However, it breaks if the dependency later
	// becomes on the controller and another worker pool (e.g.
	// [ "module.equinixmetal-cluster", "module.worker-1"]) as the resources aren't of
...
// For example:
//	// target will be "module.equinixmetal-clusterName.ex".
//	target := clusterTarget("clusterName", "ex")
//nolint: unparam
func clusterTarget(name, resource string) string {
	return fmt.Sprintf("module.equinixmetal-%v.%v", name, resource)
}
