...
component "flatcar-linux-update-operator" {}
	...
	file, diags := hclParser.ParseHCL([]byte(configHCL), fmt.Sprintf("%s.lokocfg", componentName))
	if diags.HasErrors() {
	...
	diagnostics = component.LoadConfig(c.LoadComponentConfigBody(componentName), &hcl.EvalContext{})
	if diagnostics.HasErrors() {
