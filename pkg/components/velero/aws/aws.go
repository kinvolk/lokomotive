...
type BackupStorageLocation struct {
	...
	S3ForcePathStyle string `hcl:"s3_force_path_style,optional"`
...
func (b *BackupStorageLocation) validate() hcl.Diagnostics { // nolint:dupl
	...
	if b == nil {
		...
		diagnostics = append(diagnostics, &hcl.Diagnostic{
			...
			Summary:  "'aws.backup_storage_location' block must be specified",
	...
	if b.Bucket == "" {
		diagnostics = append(diagnostics, &hcl.Diagnostic{
			Severity: hcl.DiagError,
			Summary:  "'aws.backup_storage_location.bucket' cannot be empty",
			Detail:   "Make sure to set the field to valid non-empty value",
		})
	}
	...
	if b.Region == "" {
		diagnostics = append(diagnostics, &hcl.Diagnostic{
			Severity: hcl.DiagError,
			Summary:  "'aws.backup_storage_location.region' cannot be empty",
			Detail:   "Make sure to set the field to valid non-empty value",
		})
	}
...
func (v *VolumeSnapshotLocation) validate() hcl.Diagnostics { // nolint:dupl
	var diagnostics hcl.Diagnostics
	...
	if v == nil {
		...
		diagnostics = append(diagnostics, &hcl.Diagnostic{
			Severity: hcl.DiagError,
			Summary:  "'aws.volume_snapshot_location' block must be specified",
			Detail:   "Make sure to set the field to valid non-empty value",
		})
	}
	...
	if v.Region == "" {
		diagnostics = append(diagnostics, &hcl.Diagnostic{
			Severity: hcl.DiagError,
			Summary:  "'aws.volume_snapshot_location.region' cannot be empty",
			Detail:   "Make sure to set the field to valid non-empty value",
		})
	}
