...
func renderManifests(configHCL string) (map[string]string, error) {
	component, err := components.Get("headlamp")
	if err != nil {
		return nil, err
	}

	body, diagnostics := util.GetComponentBody(configHCL, "headlamp")
	if diagnostics != nil {
...
func checkHeadlampDeploymentNotEmpty(t *testing.T, m map[string]string) {
...
component "headlamp" {
  ingress {
    host = "headlamp.test.example.com"
  }
}
`,
			false,
...
spec:
  tls:
    - hosts:
        - "headlamp.test.example.com"
      secretName: headlamp.test.example.com-tls
  rules:
    - host: "headlamp.test.example.com"
      http:
        paths:
...
spec:
  ...
  rules:
	...
	testFunc := func(t *testing.T, tc testCase) {
		...
		checkHeadlampDeploymentNotEmpty(t, m)
