...
func bgpSessionsForNode(t *testing.T, labelSelector string) []packngo.BGPSession {
	...
	if err != nil {
		t.Fatalf("Creating Equinix Metal API client: %v", err)
	}

	projectID := os.Getenv("EQUINIXMETAL_PROJECT_ID")
	if projectID == "" {
		t.Fatalf("Equinix Metal project ID can't be empty. Is %q environment variable set?", "EQUINIXMETAL_PROJECT_ID")
	}
	...
	if deviceID == "" {
		t.Fatalf("No Equinix Metal device found with hostname %q", hostname)
	}
