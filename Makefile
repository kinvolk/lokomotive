...
.PHONY: lint
lint:
	golangci-lint run --enable-all --max-same-issues=0 --max-issues-per-linter=0 --build-tags aws,packet,e2e --new-from-rev=$$(git merge-base master HEAD) --modules-download-mode=$(MOD) ./...
