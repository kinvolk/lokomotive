...
func Test_removeKubeletChart(t *testing.T) { //nolint:funlen
	tests := []struct {
		name           string
		...
		{
			name: "upgrade_kubelet",
			charts: []helm.LokomotiveChart{
				{Name: "foobar"}, {Name: "barbaz"}, {Name: "kubelet"},
			},
			upgradeKubelet: true,
			want: []helm.LokomotiveChart{
				{Name: "foobar"}, {Name: "barbaz"}, {Name: "kubelet"},
		...
		{
			// This is the possible condition when user sets the `disable_self_hosted_kubelet = true`
			// and does not ask to upgrade kubelet.
			name: "upgrade_kubelet_but_there_is_no_kubelet",
			charts: []helm.LokomotiveChart{
				{Name: "foobar"}, {Name: "barbaz"},
			},
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			if got := removeKubeletChart(tt.charts, tt.upgradeKubelet); !reflect.DeepEqual(got, tt.want) {
